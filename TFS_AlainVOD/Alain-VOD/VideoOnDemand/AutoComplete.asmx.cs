    ...
    public class AutoComplete : System.Web.Services.WebService
    {
        ...
        public string[] GetCompletionList(string prefixText, int count, string contextKey)
        {
            ...
            try
            {
                ...
                {
                    ds = repository.fnSearchBasedOnUser(searchKey, Session["LoginUserName"].ToString(), searchOn.ToString(), BasePage.CurrentLanguage == 'E' ? false : true);

                }
                else if (Session["LoginUserName"] != null && Session["IsAdmin"] != null && Convert.ToBoolean(Session["IsAdmin"].ToString()))
                {
                    ds = repository.fnSearch(searchKey, searchOn.ToString(), BasePage.CurrentLanguage == 'E' ? false : true);
                }
