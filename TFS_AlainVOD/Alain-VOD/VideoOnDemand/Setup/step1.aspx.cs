    ...
    public partial class DatabaseSetup : BasePage
    {
        ...
        protected void btnNext_ServerClick(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string dataSource = (string.IsNullOrEmpty(HttpUtility.HtmlEncode(txtPort.Text.Trim()))) ? HttpUtility.HtmlEncode(txtIPAddress.Text.Trim()) : HttpUtility.HtmlEncode(txtIPAddress.Text.Trim()) + ',' + HttpUtility.HtmlEncode(txtPort.Text.Trim());
                var ConnectionStatus = TestConnectionString(dataSource, ref returnMessage);
                ...
                if (ConnectionStatus)
                {
                    ...
                    if (CreateNewDatabase())
                    {
                        ...
                        Response.Redirect("~/Setup/step2.aspx", false);
                    }
                    else
                    {
        ...
        private bool CreateNewDatabase()
        {
            try
            {
                ...
                string returnFromLogin = string.Empty;
                if (DbCreated)
                {
                    if (CreateLoginUser(paramlist, ref returnFromLogin))
                    {
                        if (ClsConnectionString.WriteConnectionStringToFile(txtDatabaseName.Text.Trim(), txtIPAddress.Text.Trim(), txtPort.Text.Trim(), txtUserId.Text.Trim(), txtPassword.Text.Trim(), ClsConnectionString.GetProvider()))
                        {
                    ...
                    else
                    {
                        ...
                        lblMessage.Text = returnFromLogin;
                        sb.Append(@"<script type='text/javascript'>");
                        sb.Append("$('#alertMessageModal2').modal('show');");
                        sb.Append(@"</script>");
                        ScriptManager.RegisterStartupScript(this, GetType(), "NoLoginCreated", sb.ToString(), false);
                        return false;
                    }
                }
                return false;
        ...
        private bool ExecuteDatabaseScript()
        {
            ...
            try
            {
                using (SqlConnection dbCon = new SqlConnection(ClsConnectionString.getConnectionString()))
                {
                    ...
                    FileInfo file = new FileInfo(Server.MapPath("~/App_Data/VODScript.sql"));
        ...
        private bool CreateLoginUser(DatabaseParam DBParam, ref string returnMessage)
        {
            System.Data.SqlClient.SqlConnection tmpConn;
            ...
            catch (SqlException sqle)
            {
                if (sqle.Number == 1801)
                {
                    returnMessage = Resources.Setup.DBS_Connection_Already_Exists;
                }
                else if (sqle.Number == 15025)
                {
                    returnMessage = Resources.Setup.DBS_MSG_LoginAlreadyExists;
                }
                return false;
            }
            catch (System.Exception ex)
        ...
        {
            //var connectionValid = repository.GetValidConnectionString(
            //    HttpUtility.HtmlEncode(txtIPAddress.Text.Trim()),
            //    HttpUtility.HtmlEncode(txtDatabaseName.Text.Trim()),
            //    HttpUtility.HtmlEncode(txtPort.Text.Trim()),
            //    HttpUtility.HtmlEncode(txtUserId.Text.Trim()),
            //    HttpUtility.HtmlEncode(txtPassword.Text.Trim())
            //    );

            string returnMessage = string.Empty;
            string dataSource = (string.IsNullOrEmpty(HttpUtility.HtmlEncode(txtPort.Text.Trim()))) ? HttpUtility.HtmlEncode(txtIPAddress.Text.Trim()) : HttpUtility.HtmlEncode(txtIPAddress.Text.Trim()) + ',' + HttpUtility.HtmlEncode(txtPort.Text.Trim());
            ViewState["Password"] = HttpUtility.HtmlEncode(txtPassword.Text.Trim());
            txtPassword.Attributes.Add("value", ViewState["Password"].ToString());
            var ConnectionStatus = TestConnectionString(dataSource, ref returnMessage);

            if (ConnectionStatus)
            {
                if (CheckDataBaseAlreadyExists(dataSource, HttpUtility.HtmlEncode(txtDatabaseName.Text.Trim())))
                    returnMessage = returnMessage + " and " + Resources.Setup.DBS_Connection_Already_Exists;
                StringBuilder sb = new System.Text.StringBuilder();
            ...
            else
            {
                ...
                sb.Append("$('#alertMessageModal2').modal('show');");
                sb.Append(@"</script>");
                //ClientScript.RegisterStartupScript(GetType(), "ValidConnec", sb.ToString(), false);
                ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "ShowConnectionresultModal", sb.ToString(), false);
            }
        }

        private bool CheckDataBaseAlreadyExists(string DataSource, string DatabaseName)
        {
            string DbString = "SELECT name FROM master.dbo.sysdatabases WHERE ('[' + name + ']' = '" + DatabaseName + "' OR name =  '" + DatabaseName + "')";

            System.Data.OleDb.OleDbConnection tmpConn;

            string sqlCreateDBQuery;
            tmpConn = new OleDbConnection();
            bool result = false;
            string strDefaultConnectionString = string.Format(ClsConnectionString.DefaultDBConnection(), DataSource);
            tmpConn.ConnectionString = strDefaultConnectionString; 
            ...
            try
            {
                ...
                DataSet ds = OledbHelper.ExecuteDataset(tmpConn, CommandType.Text, DbString);
                if (ds != null && ds.Tables[0].Rows.Count > 0)
                    return true;
                else return false;
        ...
        private bool TestConnectionString(string dataSource, ref string returnMessage)
        {


            string strDefaultConnectionString = string.Format(ClsConnectionString.DefaultDBConnection(), dataSource);

            SqlConnectionStringBuilder conBuild = new SqlConnectionStringBuilder(strDefaultConnectionString);

            using (SqlConnection sqlCon = new SqlConnection(conBuild.ConnectionString))
            {
                try
                {
                    sqlCon.Open();
                    returnMessage = "Connection is valid";
                    return true;
                }
                catch (SqlException ex)
                {
                    ...
                    if (ex.Number == 10061)
                    {
                        returnMessage = "The Server was not found or not accessible";
                    }
                    else if (ex.Number == 18456)
                    {
                        returnMessage = "Login failed for the default User";
                    }
                    else if (ex.Number == 18452)
                    {
                        returnMessage = " Login failed. The login is from an untrusted domain and cannot be used with Windows authentication";
                    }
                    else
                        returnMessage = ex.Message;
                    return false;
